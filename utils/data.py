...
def yes_no_only_filter(question, annotation) -> bool:
    answer = annotation["multiple_choice_answer"]
    return answer == "yes" or answer == "no"


def single_word_answer_filter(question, annotation) -> bool:
    answer = annotation["multiple_choice_answer"]
...
class VQA2Dataset(Dataset):
    ...
    def __init__(
        self,
        root: str,
        group: Union[Literal["train"], Literal["val"], Literal["test"]] = "train",
        filter_func: Callable[
            [Dict[str, Any], Dict[str, Any]], bool
        ] = single_word_answer_filter,
        image_transform: Optional[Callable] = None,
        question_transform: Optional[Callable] = None,
        question_transform_factory: Optional[
            Callable[[Iterable[str]], Callable]
        ] = None,
        answer_transform: Optional[Callable] = None,
        answer_transform_factory: Optional[Callable[[Iterable[str]], Callable]] = None,
        download: bool = False,
        val_test_split: int = 174400,
    ) -> None:
        """
        Args:
            root (str): Root directory of dataset
            group (str): train, val or test
            ...
            question_transform_factory (callable, optional): Optional tranform factory to override question_transform;
                the factory function should take in training corpus(list of str) and return a question_transform
                function.
            answer_transform (callable, optional): Optional transform to be applied on the input answer string.
            answer_transform_factory (callable, optional): Optional tranform factory to override answer_transform;
                the factory function should take in training corpus(list of str) and return a answer_transform
                function.
    ...
    def data_group(self) -> str:
        return "train" if self.group == "train" else "val"

    @property
    def coco_path(self) -> str:
        return os.path.join(self.root, f"{self.data_group}2014.zip")

    @property
    def questions_path(self) -> str:
        filename = f"v2_OpenEnded_mscoco_{self.data_group}2014_questions.json"
        return os.path.join(self.root, filename)

    @property
    def annotations_path(self) -> str:
        filename = f"v2_mscoco_{self.data_group}2014_annotations.json"
        return os.path.join(self.root, filename)

    def _check_exists(self) -> bool:
        return (
            os.path.exists(self.coco_path)
    ...
    def _get_image(self, image_id: int) -> Image.Image:
        filename = "{0}2014/COCO_{0}2014_{1:012}.jpg".format(self.data_group, image_id)
